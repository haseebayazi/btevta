        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-bold text-green-600 text-lg">{{ $stats['complete_docs'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">Incomplete</span>
                    <span class="font-bold text-yellow-600 text-lg">{{ $stats['incomplete_docs'] ?? 0 }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-bold text-blue-600 text-lg">{{ $stats['total_pending'] ?? 0 }}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            @if($pendingRegistrations->count() > 0)
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        @foreach($pendingRegistrations as $candidate)
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    @if($candidate->undertakings_count > 0)
                                        <span class="inline-block px-2 py-1 rounded text-xs font-semibold bg-green-100 text-green-800">
                                            Signed
                                        </span>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex space-x-2">
                                        ...
                                        <a href="{{ route('registration.edit', $candidate->id) }}" 
