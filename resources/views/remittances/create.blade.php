...
@section('title', 'Record New Remittance - ' . config('app.name'))
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Record New Remittance</h1>
            <p class="text-gray-600 mt-1">Track a new remittance inflow from deployed worker</p>
        </div>
    ...
    <!-- Form -->
    <form action="{{ route('remittances.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <input type="hidden" name="departure_id" id="departure_id" value="{{ old('departure_id') }}">
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="transfer_method"
                        ...
                        <option value="{{ $key }}" {{ old('transfer_method') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="primary_purpose" required
                        ...
                        <option value="{{ $key }}" {{ old('primary_purpose') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="purpose_description" rows="2"
                              ...
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 @error('purpose_description') border-red-500 @enderror">{{ old('purpose_description') }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="currency"
                        ...
                        <option value="{{ $code }}" {{ old('currency', 'PKR') == $code ? 'selected' : '' }}>{{ $code }} – {{ $name }}</option>
                        @endforeach
                    </select>
                    @error('currency')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        <input type="checkbox" id="has_foreign_currency" {{ old('amount_foreign') ? 'checked' : '' }}>
                        Foreign Currency Amount
                    </label>
                    <input type="number" name="amount_foreign" id="amount_foreign" value="{{ old('amount_foreign') }}"
                           step="0.01" min="0" placeholder="0.00"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 @error('amount_foreign') border-red-500 @enderror"
                           {{ old('amount_foreign') ? '' : 'disabled' }}>
                ...
                <div>
                    ...
                    <select name="foreign_currency" id="foreign_currency"
                        ...
                        @foreach(config('remittance.currencies') as $code => $name)
                        <option value="{{ $code }}" {{ old('foreign_currency') == $code ? 'selected' : '' }}>{{ $code }} – {{ $name }}</option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="sender_location" value="{{ old('sender_location') }}"
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="receiver_account" id="receiver_account" value="{{ old('receiver_account') }}"
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div>
                <textarea name="notes" rows="4"
                          ...
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                @error('notes')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="flex items-center justify-end space-x-3">
            <a href="{{ route('remittances.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg">
                Cancel
            </a>
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg">
                <i class="fas fa-save mr-2"></i>Record Remittance
            </button>
        </div>
    </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    candidateSelect.addEventListener('change', function() {
        ...
        // Auto-fill sender name with candidate name
        if (candidateId) {
            senderNameInput.value = selectedOption.text.split(' (')[0];
        }
