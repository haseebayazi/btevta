...
<div class="container mx-auto px-4 py-6 max-w-4xl">
    <div class="mb-6">
        <h1 class="text-3xl font-bold">Edit Instructor</h1>
        <p class="text-gray-600 mt-1">Update details for: {{ $instructor->name }}</p>
    </div>
        ...
        <form action="{{ route('instructors.update', $instructor) }}" method="POST">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="cnic" name="cnic" value="{{ old('cnic', $instructor->cnic) }}"
                    ...
                    <select id="campus_id" name="campus_id" class="form-select w-full">
                        ...
                        @foreach($campuses as $campus)
                            <option value="{{ $campus->id }}"
                                {{ old('campus_id', $instructor->campus_id) == $campus->id ? 'selected' : '' }}>
                                {{ $campus->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="trade_id" name="trade_id" class="form-select w-full">
                        ...
                        @foreach($trades as $trade)
                            <option value="{{ $trade->id }}"
                                {{ old('trade_id', $instructor->trade_id) == $trade->id ? 'selected' : '' }}>
                                {{ $trade->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="employment_type" name="employment_type" class="form-select w-full" required>
                        <option value="permanent" {{ old('employment_type', $instructor->employment_type) == 'permanent' ? 'selected' : '' }}>Permanent</option>
                        <option value="contract" {{ old('employment_type', $instructor->employment_type) == 'contract' ? 'selected' : '' }}>Contract</option>
                        <option value="visiting" {{ old('employment_type', $instructor->employment_type) == 'visiting' ? 'selected' : '' }}>Visiting</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="joining_date" name="joining_date"
                           value="{{ old('joining_date', $instructor->joining_date?->format('Y-m-d')) }}"
                           class="form-input w-full">
                </div>
                ...
                <div>
                    ...
                    <select id="status" name="status" class="form-select w-full" required>
                        ...
                        <option value="on_leave" {{ old('status', $instructor->status) == 'on_leave' ? 'selected' : '' }}>On Leave</option>
                        <option value="terminated" {{ old('status', $instructor->status) == 'terminated' ? 'selected' : '' }}>Terminated</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="address" name="address" rows="3"
                              class="form-input w-full">{{ old('address', $instructor->address) }}</textarea>
                </div>
            </div>
            ...
            <div class="flex gap-3 mt-6">
                ...
                <a href="{{ route('instructors.show', $instructor) }}" class="btn btn-info">
